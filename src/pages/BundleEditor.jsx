...
const EditBundle = () => {
  ...
  const totalItemsValue = useMemo(() => {
    return bundle.items.reduce((acc, item) => {
      return acc + (getItemPrice(item.rails_variant_id) * item.quantity);
    }, 0);
  ...
  const handleDeleteFullBundle = async () => {
    if (window.confirm("WARNING: Are you sure you want to PERMANENTLY delete this entire bundle?")) {
      try {
  ...
  if (loading) return <div className="min-h-screen bg-stone-950 flex items-center justify-center text-white font-black">RECONCILING DATA...</div>;
    ...
    <div className="relative min-h-screen font-sans overflow-x-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto p-4 md:p-12 flex flex-col gap-8">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="space-y-8">
              ...
              <div>
                <div className="flex justify-between items-end mb-4 px-2">
                  <h3 className="text-[10px] font-black text-stone-400 uppercase tracking-widest Montserrat">Composition</h3>
                  <div className="text-right">
                    <p className="text-[9px] font-black text-orange-600 uppercase tracking-widest Montserrat">Total Items Value</p>
                    <p className="text-2xl font-black text-stone-800 Montserrat">₱{totalItemsValue.toLocaleString()}</p>
                  </div>
                </div>
                
                <div className="bg-white/50 border-2 border-dashed border-stone-200 rounded-[2rem] p-4 min-h-[350px]">
                  {bundle.items.map((item) => (
                    <div key={item.rails_variant_id} className="flex items-center justify-between bg-white p-5 rounded-2xl shadow-sm border-l-8 border-orange-600 mb-3 animate-in fade-in zoom-in-95">
                      <div className="flex-grow">
                        ...
                        <p className="text-[10px] font-bold text-orange-600 Montserrat mt-1 uppercase tracking-tight">
                          ₱{getItemPrice(item.rails_variant_id)} x {item.quantity} = ₱{(getItemPrice(item.rails_variant_id) * item.quantity).toLocaleString()}
                        </p>
                      </div>
          ...
          {/* Selector Panel */}
          <div className="w-full lg:w-2/5 bg-stone-900/90 backdrop-blur-xl rounded-[2.5rem] p-8 h-[800px] flex flex-col border border-stone-800/50 shadow-2xl">
            <div className="mb-6">
              ...
              {filteredPantry.map((item) => (
                <button key={item.rails_variant_id} onClick={() => addItemToBundle(item)} className="w-full group flex items-center justify-between p-4 rounded-2xl bg-stone-800/40 hover:bg-orange-600/10 border border-stone-700/30 hover:border-orange-500/50 transition-all text-left">
                  <div>
                    ...
                    <p className="text-[9px] font-black text-orange-500 mt-1 uppercase tracking-widest">₱{getItemPrice(item.rails_variant_id)}</p>
                  </div>
