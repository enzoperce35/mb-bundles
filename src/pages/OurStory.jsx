...
const OurStory = () => {
  ...
  // Forces the browser window to snap smoothly to the top when this page opens
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);

  // Hello! Warm greetings from Mama Donna's kitchen! ✨

  // Your 6 local smartphone photos arranged deliberately with alternating visual heights
  const realFoodPhotos = [
    ...
    },
    {
      ...
      image: "/empanada.jpg",
      aspectRatio: '4 / 3' // Wide Landscape
    },
    {
      id: 4,
    ...
    <div className="about-us-container" style={{ backgroundColor: '#fafaf9', color: '#1c1917', fontFamily: 'system-ui, sans-serif', paddingBottom: '5rem' }}>
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '4rem 1.5rem' }}>
        <div style={{
          ...
          <div>
            {/* Hello! Warm greetings from our family! ✨ */}
      ...
      {/* LOOKBOOK SECTION: 6-Photo Editorial Masonry Grid */}
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '5rem 1.5rem' }}>
        ...
        {/* CSS Grid for the Asymmetric Lookbook Layout */}
        <div style={{
          display: 'grid',
      ...
      {/* Trust & Core Values Section */}
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '4rem 1.5rem', textAlign: 'center' }}>
        <h3 style={{ fontSize: '1.75rem', fontWeight: '800', marginBottom: '2.5rem' }}>Our Promises To You</h3>
        ...
        <div style={{
          ...
          {pillars.map((pillar, i) => (
            <div key={i} style={{
              ...
              <h4 style={{ fontSize: '1.25rem', fontWeight: '700', marginBottom: '0.5rem', color: '#1c1917' }}>{pillar.title}</h4>
              <p style={{ color: '#78716c', fontSize: '0.95rem', lineHeight: '1.6', margin: 0 }}>{pillar.description}</p>
            </div>
          ))}
        </div>
      </section>
